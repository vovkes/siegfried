...
func CheckTransactionSanity(tx *wire.MsgTx, params *chaincfg.Params) error {
	...
	// known as an atom.  One wbcoin is a quantity of atoms as defined by
	...
	for _, txOut := range tx.TxOut {
		...
		// is detected and reported.  This is impossible for WBC,
...
//
// WBC: Check the stake transactions to make sure they don't have this txid
// too.
...
// owner of the wbcoin and therefore allowed to spend them.  As it checks the
...
func CheckTransactionInputs(subsidyCache *SubsidyCache, tx *dcrutil.Tx, txHeight int64, utxoView *UtxoViewpoint, checkFraudProof bool, chainParams *chaincfg.Params) (int64, error) {
	...
	// -------------------------------------------------------------------
	// WBC general transaction testing (and a few stake exceptions).
	// -------------------------------------------------------------------
	for idx, txIn := range msgTx.TxIn {
		...
		// wbcoin is a quantity of atoms as defined by the AtomPerCoin
		// constant.
