...
const (
	// svcName is the name of wbc service.
	svcName = "wbcsvc"
	...
	svcDesc = "Downloads and stays synchronized with the wbcoin block " +
...
// logServiceStartOfDay logs information about wbc when the main server has
...
// wbcService houses the main service handler which handles all service
// updates and launching wbcMain.
type wbcService struct{}
...
// long-running wbcMain (which is the real meat of wbc), handles service
// change requests, and notifies the service control manager of changes.
func (s *wbcService) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (bool, uint32) {
	// Service start is pending.
	const cmdsAccepted = svc.AcceptStop | svc.AcceptShutdown
	changes <- svc.Status{State: svc.StartPending}

	// Start wbcMain in a separate goroutine so the service can start
...
// installService attempts to install the wbc service.  Typically this should
...
// removeService attempts to uninstall the wbc service.  Typically this should
...
// startService attempts to start the wbc service.
...
func serviceMain() (bool, error) {
	...
	err = svc.Run(svcName, &wbcService{})
	if err != nil {
