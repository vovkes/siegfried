...
/*
Package wbcutil provides wbcoin-specific convenience functions and types.

Block Overview

A Block defines a wbcoin block that provides easier and more efficient
...
A Tx defines a wbcoin transaction that provides more efficient manipulation of
...
The Address interface provides an abstraction for a WBC address.  While the
most common type is a pay-to-pubkey-hash, WBC already supports others and
...
To decode/encode an address:
	...
	addr, err := wbcutil.DecodeAddress(addrString, defaultNet)
	if err != nil {
		fmt.Println(err)
		return
	}
