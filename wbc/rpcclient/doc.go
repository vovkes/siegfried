...
/*
Package rpcclient implements a websocket-enabled WBC JSON-RPC client.
...
wbc or wbcwallet by default.  However, configuration options are provided to
...
In contrast, the websocket-based JSON-RPC interface provided by wbc and
wbcwallet only uses a single connection that remains open and allows
...
All notifications provided by wbc require registration to opt-in.  For example,
...
example, the DebugLevel call is an extension only provided by wbc (and
wbcwallet passthrough).  Therefore if you call one of these commands against
...
Also, it is important to realize that wbc intentionally separates the wallet
functionality into a separate process named wbcwallet.  This means if you are
connected to the wbc RPC server directly, only the RPCs which are related to
chain services will be available.  Depending on your application, you might only
need chain-related RPCs.  In contrast, wbcwallet provides pass through treatment
...
The following full-blown client examples are in the examples directory:
 ...
 - wbcwebsockets
   ...
   block count
 - dcrwalletwebsockets
   Connects to a wbcwallet RPC server using TLS-secured websockets, registers
