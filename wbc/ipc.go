...
// processes to communicate with and and manage the lifetime of a wbc child
...
// message, shutdown begins.  This prevents wbc from continuing to run
...
func serviceControlPipeRx(fd uintptr) {
	...
	for {
		...
		if err != nil {
			wbcLog.Errorf("Failed to read from pipe: %v", err)
			break
		}
	}
...
// for a child wbc process to communicate with a parent process without the
...
func serviceControlPipeTx(fd uintptr) {
	...
	wbcLog.Errorf("Failed to write to pipe: %v", err)
}
