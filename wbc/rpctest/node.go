...
// arguments returns an array of arguments that be used to launch the wbc
// process.
...
// command returns the exec.Cmd which will be used to start the wbc process.
...
// to the wbc process that is launched via Start().
...
// as the base for the log and data directories for wbc.
...
// start creates a new wbc process, and writes its pid in a file reserved for
...
func (n *node) start() error {
	...
	pid, err := os.Create(filepath.Join(n.config.String(), "wbc.pid"))
	if err != nil {
		return err
	}
...
// stop interrupts the running wbc process process, and waits until it exits
...
// shutdown terminates the running wbc process, and cleans up all
