...
// MessageHeaderSize is the number of bytes in a wbcoin message header.
// WBC network (magic) 4 bytes + command 12 bytes + payload length 4 bytes +
// checksum 4 bytes.
const MessageHeaderSize = 24

// CommandSize is the fixed size of all commands in the common wbcoin message
...
// Message is an interface that describes a wbcoin message.  A type that
...
// messageHeader defines the header structure for all wbcoin protocol messages.
...
// readMessageHeader reads a wbcoin message header from r.
...
// WriteMessageN writes a wbcoin Message to w including the necessary header
...
// WriteMessage writes a wbcoin Message to w including the necessary header
...
// ReadMessageN reads, validates, and parses the next wbcoin Message from r for
// the provided protocol version and wbcoin network.  It returns the number of
...
func ReadMessageN(r io.Reader, pver uint32, dcrnet CurrencyNet) (int, Message, []byte, error) {
	...
	// Check for messages from the wrong wbcoin network.
	if hdr.magic != dcrnet {
...
// ReadMessage reads, validates, and parses the next wbcoin Message from r for
// the provided protocol version and wbcoin network.  It returns the parsed
