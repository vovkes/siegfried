...
// FileContents is a string containing the commented example config for wbc.
...
; The default is ~/.wbc/data on POSIX OSes, $LOCALAPPDATA/Dcrd/data on Windows,
; ~/Library/Application Support/Dcrd/data on Mac OS, and $homed/wbc/data on
...
; line.  wbc will not contact 3rd-party sites to obtain external ip addresses.
...
; with, but the behavior is slightly different.  By default, wbc will query DNS
...
; Disable DNS seeding for peers.  By default, when wbc starts, it will use
...
; which is used to control and query information from a running wbc process.
;
...
; log level for individual subsystems.  Use wbc --debuglevel=show to list
