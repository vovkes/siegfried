...
// blockMsg packages a wbcoin block message and the peer it came from together
...
// invMsg packages a wbcoin inv message and the peer it came from together
// so the block handler has access to that information.
type invMsg struct {
	inv  *wire.MsgInv
	peer *serverPeer
...
// headersMsg packages a wbcoin headers message and the peer it came from
...
// txMsg packages a wbcoin tx message and the peer it came from together
// so the block handler has access to that information.
type txMsg struct {
	tx   *dcrutil.Tx
	peer *serverPeer
...
// newBlockManager returns a new wbcoin block manager.
...
func warnMultipleDBs() {
	...
	if len(duplicateDbPaths) > 0 {
		...
		wbcLog.Warnf("WARNING: There are multiple block chain databases "+
...
func loadBlockDB() (database.DB, error) {
	...
	if cfg.DbType == "memdb" {
		wbcLog.Infof("Creating block database in memory.")
	...
	wbcLog.Infof("Loading block database from '%s'", dbPath)
	...
	wbcLog.Info("Block database loaded")
	return db, nil
}
