...
var (
	wbcHomeDir             = dcrutil.AppDataDir("wbc", false)
	...
	defaultRPCCertFile     = filepath.Join(wbcHomeDir, "rpc.cert")
...
func listCommands() {
	...
	for _, method := range cmdMethods {
		flag, err := dcrjson.MethodUsageFlags(method)
		if err != nil {
		...
		// Categorize the command based on the usage flag.
		category := categoryChain
		if flag&dcrjson.UFWalletOnly != 0 {
...
// For this it tries to read the wbc config file at its default path, and extract
// the RPC user and password from it.
func createDefaultConfigFile(destinationPath string) error {
	// Nothing to do when there is no existing wbc conf file at the default
	// path to extract the details from.
	wbcConfigPath := filepath.Join(wbcHomeDir, "wbc.conf")
	if !fileExists(wbcConfigPath) {
		return nil
	}

	// Read wbc.conf from its default path
	wbcConfigFile, err := os.Open(wbcConfigPath)
	if err != nil {
		return err
	}
	defer wbcConfigFile.Close()
	content, err := ioutil.ReadAll(wbcConfigFile)
	if err != nil {
		return err
	}
