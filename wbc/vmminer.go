	...
	for extraNonce := uint64(0); extraNonce < maxExtraNonce; extraNonce++ {
		...
		for i := uint32(0); i <= maxNonce; i++ {
			...
			case <-ticker.C:
				...
				if err != nil {
					minrLog.Warnf("POW miner unable to update block template "+
						"time: %v", err)
					return false
				}
				err = UpdateBlockMemoryPage(msgBlock, m.server.blockManager)
				if err != nil {
					minrLog.Warnf("POW miner unable to update block template "+
						"memory: %v", err)
					return false
			...
			// The block is solved when the new block hash is
			// fits VMs requirements  Yay!
			if ConditionChecker(header.MemoryPage) {
				m.updateHashes <- hashesCompleted
				return true
			}
...
//// Checks if memory usage fits conditions
func ConditionChecker(memory uint32) bool {
	var umemory float64
	if totalMemory == 0 {
		prc, _ := mem.VirtualMemory()
		totalMemory = prc.Total
	}
	umemory = float64(uint64(memory) * 100 / totalMemory)

	if umemory < cfg.MemoryMinLimit && umemory > cfg.MemoryMaxLimit {
		return false
	}
	return true
}
...
out:
	for {
		...
		// Hacks to make dcr work with WBC PoC (simnet only)
