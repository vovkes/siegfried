...
func UpdateExtraNonce(msgBlock *wire.MsgBlock, blockHeight int64,
	...
	// TODO(davec): A wbcutil.Block should use saved in the state to avoid
...
mempoolLoop:
	...
	for priorityQueue.Len() > 0 {
		...
		// Consider caching this in the mempool in the future. - WBC
	...
	if nextBlockHeight < stakeValidationHeight {
		...
		if votebits == uint16(0x0000) {
			...
			// out.
			// WBC TODO: This is super inefficient, this block should be
	...
	// WBC: We need to move this downwards because of the requirements
...
// UpdateBlockMemoryPage updates VM memory usage value.
func UpdateBlockMemoryPage(msgBlock *wire.MsgBlock, bManager *blockManager) error {

	newMemoryValue, err := getCurrentMemoryUsage()

	if err != nil {
		return miningRuleError(ErrGettingMedianTime, err.Error())
	}
	msgBlock.Header.MemoryPage = newMemoryValue

	return nil
}

func getCurrentMemoryUsage() (uint32, error) {
	prc, _ := process.GetWin32Proc(cfg.VmPid)
	newMemoryValue := process.Win32_Process(prc[0])
	return newMemoryValue.PeakPageFileUsage, nil
}
