...
import (
	...
	"bitbucket.org/siegfriedvmblockchain/siegfried/wbc/mining"
	"bitbucket.org/siegfriedvmblockchain/siegfried/wbc/peer"
...
var (
	...
	// ourselves to other wbcoin peers.
	userAgentName = "wbc"
...
// broadcastMsg provides the ability to house a wbcoin message to be broadcast
...
// server provides a wbcoin server for handling communications to and from
// wbcoin peers.
type server struct {
...
func (sp *serverPeer) OnTx(p *peer.Peer, msg *wire.MsgTx) {
	...
	// Convert the raw MsgTx to a wbcutil.Tx which provides some convenience
...
func (sp *serverPeer) OnBlock(p *peer.Peer, msg *wire.MsgBlock, buf []byte) {
	// Convert the raw MsgBlock to a wbcutil.Block which provides some
...
func (s *server) peerHandler() {
	...
	if !cfg.DisableDNSSeed {
		...
		connmgr.SeedFromDNS(activeNetParams.Params, defaultRequiredServices, wbcLookup, func(addrs []*wire.NetAddress) {
...
// newServer returns a new wbc server configured to listen on addr for the
// wbcoin network type specified by chainParams.  Use start to begin accepting
...
func newServer(listenAddrs []string, db database.DB, chainParams *chaincfg.Params) (*server, error) {
	...
	amgr := addrmgr.New(cfg.DataDir, wbcLookup)
...
func addrStringToNetAddr(addr string) (net.Addr, error) {
	...
	// The wbcLookup function will transparently handle performing the
	// lookup over Tor if necessary.
	ips, err := wbcLookup(host)
	if err != nil {
		return nil, err
	}
	if len(ips) == 0 {
