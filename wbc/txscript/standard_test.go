...
// newAddressPubKey returns a new wbcutil.AddressPubKey from the provided
...
// newAddressPubKeyHash returns a new wbcutil.AddressPubKeyHash from the
...
// newAddressScriptHash returns a new wbcutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
...
// bogusAddress implements the wbcutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the wbcutil.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	tests := []struct {
		...
		// pay-to-pubkey address on mainnet. for wbcoin this would
		// be uncompressed, but standard for wbcoin is 33 byte
...
func TestMultiSigScript(t *testing.T) {
	...
	tests := []struct {
		...
		{
			// By default compressed pubkeys are used in WBC.
			[]*dcrutil.AddressSecpPubKey{
