...
wbc provides a [JSON-RPC](http://json-rpc.org/wiki/specification) API that is
fully compatible with the original bitcoind/bitcoin-qt.  There are a few key
differences between wbc and bitcoind as far as how RPCs are serviced:
* Unlike bitcoind that has the wallet and chain intermingled in the same process
  which leads to several issues, wbc intentionally splits the wallet and chain
  ...
  talking directly to wbc, only chain-related RPCs are available.  However both
  chain-related and wallet-related RPCs are available via
  [dcrwallet](https://github.com/wbcoin/dcrwallet).
* wbc is secure by default which means that the RPC connection is TLS-enabled
  by default
* wbc provides access to the API through both
  [HTTP POST](http://en.wikipedia.org/wiki/POST_%28HTTP%29) requests and
  [Websockets](http://en.wikipedia.org/wiki/WebSocket)

Websockets are the preferred transport for wbc RPC and are used by applications
such as [dcrwallet](https://github.com/wbcoin/dcrwallet) for inter-process
communication with wbc.  The websocket connection endpoint for wbc is
...
The wbc RPC server supports both [HTTP POST](http://en.wikipedia.org/wiki/POST_%28HTTP%29)
...
wbc is `wss://your_ip_or_domain:9109/ws`.
...
* **rpcuser** is the full-access username configured for the wbc RPC server
* **rpcpass** is the full-access password configured for the wbc RPC server
* **rpclimituser** is the limited username configured for the wbc RPC server
* **rpclimitpass** is the limited password configured for the wbc RPC server
* **rpccert** is the PEM-encoded X.509 certificate (public key) that the wbc
  server is configured with.  It is automatically generated by wbc and placed
  in the wbc home directory (which is typically `%LOCALAPPDATA%\Dcrd` on
  Windows and `~/.wbc` on POSIX-like OSes)

**NOTE:** As mentioned above, wbc is secure by default which means the RPC
...
The wbc RPC server uses HTTP [basic access authentication](http://en.wikipedia.org/wiki/Basic_access_authentication) with the **rpcuser**
...
wbc comes with a separate utility named `dcrctl` which can be used to issue
these RPC commands via HTTP POST requests to wbc after configuring it with the
...
API compatible with the original wbc client.
...
|23|[getwork](#getwork)|N|Returns formatted hash data to work on or checks and submits solved data.<br /><br />NOTE: Since wbc does not have the wallet integrated to provide payment addresses, wbc must be configured via the `--miningaddr` option to provide which payment addresses to pay created blocks to for this RPC to function.|
...
|26|[sendrawtransaction](#sendrawtransaction)|Y|Submits the serialized, hex-encoded transaction to the local peer and relays it to the network.<br /><br />NOTE: wbc does not yet implement the `allowhighfees` parameter, so it has no effect|
|27|[setgenerate](#setgenerate) |N|Set the server to generate coins (mine) or not.<br/>NOTE: Since wbc does not have the wallet integrated to provide payment addresses, wbc must be configured via the `--miningaddr` option to provide which payment addresses to pay created blocks to for this RPC to function.|
|28|[stop](#stop)|N|Shutdown wbc.|
|29|[submitblock](#submitblock)|Y|Attempts to submit a new serialized, hex-encoded block to the network.|
|30|[validateaddress](#validateaddress)|Y|Verifies the given address is valid.  NOTE: Since wbc does not have a wallet integrated, wbc will only return whether the address is valid or not.|
...
|34|[getcurrentnet](#getcurrentnet)|Y|Get wbcoin network wbc is running on.|
...
|Returns|`(json object)`<br />`asm`: `(string)` disassembly of the script (absent for nonstandard scripts).<br />`reqSigs`: `(numeric)` the number of required signatures.<br />`type`: `(string)` the type of the script (e.g. 'pubkeyhash').<br />`addresses`: `(json array of string)` the wbcoin addresses associated with this script.<br />`p2sh`: `(string)` the script hash for use in pay-to-script-hash transactions.<br /><br />`{ "asm": "asm", "reqSigs": n, "type": "scripttype", "addresses": [...], "p2sh": "scripthash"}`|
...
|Notes|NOTE: Since wbc does NOT contain wallet functionality, wallet-related fields are not returned.  See getinfo in dcrwallet for a version which includes that information.|
...
|Example Return|`[{"addr": "178.172.xxx.xxx:9108", "services": "00000001", "lastrecv": 1388183523, "lastsend": 1388185470, "bytessent": 287592965, "bytesrecv": 780340, "conntime": 1388182973, "pingtime": 405551, "pingwait": 183023, "version": 70001, "subver": "/wbc:0.4.0/", "inbound": false, "startingheight": 276921, "currentheight": 276955, "syncnode": true }, ...]`|
[Return to Overview](#MethodOverview)<br />
...
|Description|Returns information about a transaction given its hash.|
|Notes|Since wbc does not have the wallet integrated to provide payment addresses, wbc must be configured via the `--miningaddr` option to provide which payment addresses to pay created blocks to for this RPC to function.
...
|Notes|Since wbc does not perform any mining, the priority related fields `startingpriority` and `currentpriority` that are available when the `verbose` flag is set are always 0.|
...
|Notes|NOTE: Since wbc does not have the wallet integrated to provide payment addresses, wbc must be configured via the `--miningaddr` option to provide which payment addresses to pay created blocks to for this RPC to function.|
|Returns|Nothing|
[Return to Overview](#MethodOverview)<br />
...
|Notes|wbc does not yet implement the `allowhighfees` parameter, so it has no effect.|
...
|Method|stop|
|Parameters|None|
|Description|Shutdown wbc.|
|Returns|`"wbc stopping."` (string)|
[Return to Overview](#MethodOverview)<br />
...
|Parameters|1. `address`: `(string, required)` wbcoin address.|
|Description|Verify an address is valid.|
|Returns|`(json object)`<br />`isvalid`: `(bool)` whether or not the address is valid.<br />`address`: `(string)` the wbcoin address validated.<br /><br />`{"isvalid": true or false,"address": "wbcoinaddress"}`|
[Return to Overview](#MethodOverview)<br />
...
|Description|Verifies the block chain database. The actual checks performed by the `checklevel` parameter is implementation specific. <br /><br />For wbc this is: <br />`checklevel=0` - Look up each block and ensure it can be loaded from the database.<br />`checklevel=1` - Perform basic context-free sanity checks on each block.|
|Notes|Dcrd currently only supports `checklevel` 0 and 1, but the default is still 3 for compatibility.  Per the information in the Parameters section above, higher levels are automatically clamped to the highest supported level, so this means the default is effectively 1 for wbc.|
...
|Parameters|None|
|Description|Get wbcoin network wbc is running on.|
|Returns|`numeric`|
...
|Parameters|1. `Addresses`: `(JSON array, required)`<br />`wbcoinaddress`: `(string)` the wbcoin address.<br /><br />`["wbcoinaddress", ...]`|
...
|Description|Send a redeemingtx notification when a transaction spending an outpoint appears in mempool (if relayed to this wbc instance) and when such a transaction first appears in a newly-attached block.|
|Returns|Nothing|
[Return to Overview](#WSMethodOverview)<br />
...
wbc uses standard JSON-RPC notifications to notify clients of changes, rather than requiring clients to poll wbc for updates.  JSON-RPC notifications are a subset of requests, but do not contain an ID.  The notification type is categorized by the `method` field and additional details are sent as a JSON array in the `params` field.
...
[rpcclient](https://github.com/wbcoin/wbc/tree/master/rpcclient) package.
...
[rpcclient](https://github.com/wbcoin/wbc/tree/master/rpcclient) package to connect with
a wbc instance via Websockets, issues [getblockcount](#getblockcount) to
...
func main() {
	...
	// generated by wbc when it starts the RPC server and doesn't already
	// have one.
...
The following is an example Go application which uses the
[rpcclient](https://github.com/wbcoin/wbc/tree/master/rpcclient) package to connect with
a wbc instance via Websockets, issues [getblock](#getblock) to retrieve
...
The following is an example Go application which uses the
[rpcclient](https://github.com/wbcoin/wbc/tree/master/rpcclient) package to connect with
a wbc instance via Websockets and registers for
...
(can be installed with `npm install ws`) to connect with a wbc instance,
...
// generated by wbc when it starts the RPC server and doesn't already
// have one.
var cert = fs.readFileSync('/path/to/wbc/appdata/rpc.cert');
var user = "yourusername";
var password = "yourpassword";


// Initiate the websocket connection.  The wbc generated certificate acts as
