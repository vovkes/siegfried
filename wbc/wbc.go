...
// winServiceMain is only invoked on Windows.  It detects when wbc is running
// as a service and reacts accordingly.
var winServiceMain func() (bool, error)

// wbcMain is the real main function for wbc.  It is necessary to work around
// the fact that deferred functions do not run when os.Exit() is called.  The
// optional serverChan parameter is mainly used by the service code to be
// notified with the server once it is setup so it can gracefully stop it when
// requested from the service control manager.
func wbcMain(serverChan chan<- *server) error {
	...
	defer wbcLog.Info("Shutdown complete")

	// Show version and home dir at startup.
	wbcLog.Infof("Version %s (Go version %s)", version(), runtime.Version())
	wbcLog.Infof("Home dir: %s", cfg.HomeDir)
	if cfg.NoFileLogging {
		wbcLog.Info("File logging disabled")
	}
	...
	if cfg.Profile != "" {
		go func() {
			...
			wbcLog.Infof("Creating profiling server "+
	...
	if cfg.CPUProfile != "" {
		...
		if err != nil {
			wbcLog.Errorf("Unable to create cpu profile: %v", err.Error())
			return err
		}
	...
	if cfg.MemProfile != "" {
		...
		if err != nil {
			wbcLog.Errorf("Unable to create cpu profile: %v", err)
			return err
		}
	...
	defer func() {
		...
		wbcLog.Infof("Gracefully shutting down the database...")
		db.Close()
	}()
	...
	if err != nil {
		...
		wbcLog.Errorf("Unable to start server on %v: %v",
			cfg.Listeners, err)
		return err
	}
	defer func() {
		lifetimeNotifier.notifyShutdownEvent(lifetimeEventP2PServer)
		wbcLog.Infof("Gracefully shutting down the server...")
		server.Stop()
