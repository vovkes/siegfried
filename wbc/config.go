...
const (
	defaultConfigFilename        = "wbc.conf"
	defaultDataDirname           = "data"
	defaultLogLevel              = "info"
	defaultLogDirname            = "logs"
	defaultLogFilename           = "wbc.log"
	defaultMaxPeers              = 125
...
var (
	defaultHomeDir     = dcrutil.AppDataDir("wbc", false)
...
// config defines the configuration options for wbc.
//
...
type config struct {
	...
	///VM
	VmPid          int32   `int:"VM process ID"`
	MemoryMaxLimit float64 `float:"VM memory upper limit in %"`
	MemoryMinLimit float64 `float:"VM memory upper limit in %"`
}
...
// createDefaultConfig copies the file sample-wbc.conf to the given destination path,
...
//
// The above results in wbc functioning properly without any config settings
...
func loadConfig() (*config, []string, error) {
	...
	// Update the home directory for wbc if specified. Since the home
	...
	for _, oldDir := range oldTestNets {
		if fileExists(oldDir) {
			wbcLog.Warnf("Block chain data from previous testnet"+
	...
	if configFileError != nil {
		wbcLog.Warnf("%v", configFileError)
	}

	return &cfg, remainingArgs, nil
}

// wbcDial connects to the address on the named network using the appropriate
// dial function depending on the address and configuration options.  For
// example, .onion addresses will be dialed using the onion specific proxy if
// one was specified, but will otherwise use the normal dial function (which
// could itself use a proxy or not).
func wbcDial(addr net.Addr) (net.Conn, error) {
...
// wbcLookup returns the correct DNS lookup function to use depending on the
...
func wbcLookup(host string) ([]net.IP, error) {
