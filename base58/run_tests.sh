...
DOCKER_IMAGE_TAG=wbcoin-golang-builder-$GOVERSION

docker pull wbcoin/$DOCKER_IMAGE_TAG

docker run --rm -it -v $(pwd):/src wbcoin/$DOCKER_IMAGE_TAG /bin/bash -c "\
  rsync -ra --filter=':- .gitignore'  \
  /src/ /go/src/github.com/wbcoin/$REPO/ && \
  cd github.com/wbcoin/$REPO/ && \
  dep ensure && \
  go install && \
  $TESTCMD
"
