...
var (
	wbcDefaultCAFile   = filepath.Join(dcrutil.AppDataDir("wbc", false), "rpc.cert")
...
type config struct {
	...
	// RPC client options
	RPCConnect       string                  `short:"c" long:"rpcconnect" description:"Hostname/IP and port of wbc RPC server to connect to"`
	CAFile           *cfgutil.ExplicitString `long:"cafile" description:"File containing root certificates to authenticate a TLS connections with wbc"`
	DisableClientTLS bool                    `long:"noclienttls" description:"Disable TLS for the RPC client -- NOTE: This is only allowed if the RPC client is connecting to localhost"`
	DcrdUsername     string                  `long:"wbcusername" description:"Username for wbc authentication"`
	DcrdPassword     string                  `long:"wbcpassword" default-mask:"-" description:"Password for wbc authentication"`
	...
	Username               string                  `short:"u" long:"username" description:"Username for legacy JSON-RPC and wbc authentication (if wbcusername is unset)"`
	Password               string                  `short:"P" long:"password" default-mask:"-" description:"Password for legacy JSON-RPC and wbc authentication (if wbcpassword is unset)"`
...
func loadConfig(ctx context.Context) (*config, []string, error) {
	...
	if cfg.DisableClientTLS {
		...
		// If CAFile is unset, choose either the copy or local wbc cert.
		if !cfg.CAFile.ExplicitlySet() {
			cfg.CAFile.Value = filepath.Join(cfg.AppDataDir.Value, defaultCAFilename)

			// If the CA copy does not exist, check if we're connecting to
			// a local wbc and switch to its RPC cert if it exists.
			...
			if !certExists {
				if _, ok := localhostListeners[RPCHost]; ok {
					wbcCertExists, err := cfgutil.FileExists(
						wbcDefaultCAFile)
					if err != nil {
						fmt.Fprintln(os.Stderr, err)
						return loadConfigError(err)
					...
					if wbcCertExists {
						cfg.CAFile.Value = wbcDefaultCAFile
					}
				}
			}
		}
	...
	// If the wbc username or password are unset, use the same auth as for
	// the client.  The two settings were previously shared for wbc and
	// client auth, so this avoids breaking backwards compatibility while
	// allowing users to use different auth settings for wbc and wallet.
	if cfg.DcrdUsername == "" {
