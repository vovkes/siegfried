...
// RPCClient represents a persistent client connection to a wbcoin RPC server
...
func (c *RPCClient) Start(ctx context.Context, retry bool) (err error) {
	...
	if err == nil {
		versionResult := versions["wbcjsonrpcapi"]
		serverAPI = semver{
...
// wbcoin RPC server.  This channel must be continually read or the process
...
out:
	for {
		...
		case <-pingChan:
			...
			// be due to wbc processing websocket requests synchronously (see
