...
dcrwallet is a daemon handling wbcoin wallet functionality for a
single user.  It acts as both an RPC client to wbc and an RPC server
...
remote wbc instance for asynchronous blockchain queries and
notifications over websockets.  Full wbc installation instructions
can be found [here](https://github.com/wbcoin/wbc).  An alternative
SPV mode that is compatible with wbc is planned for a future release.
...
https://github.com/wbcoin/wbcoin-release/releases
...
```
git clone https://github.com/wbcoin/dcrwallet $GOPATH/src/github.com/wbcoin/dcrwallet
cd $GOPATH/src/github.com/wbcoin/dcrwallet
dep ensure
```
...
to a localhost wbc.  Commands should be run in `cmd.exe` or PowerShell on
Windows, or any terminal emulator on *nix.

- Run the following command to start wbc:

```
wbc -u rpcuser -P rpcpass
```
...
copy the sample wbc and dcrwallet configurations and update with your
RPC username and password.

PowerShell (Installed from MSI):
```
PS> cp "$env:ProgramFiles\WBC\Dcrd\sample-wbc.conf" $env:LOCALAPPDATA\Dcrd\wbc.conf
PS> cp "$env:ProgramFiles\WBC\Dcrwallet\sample-dcrwallet.conf" $env:LOCALAPPDATA\Dcrwallet\dcrwallet.conf
PS> $editor $env:LOCALAPPDATA\Dcrd\wbc.conf
PS> $editor $env:LOCALAPPDATA\Dcrwallet\dcrwallet.conf
```

PowerShell (Installed from source):
```
PS> cp $env:GOPATH\src\github.com\wbcoin\wbc\sample-wbc.conf $env:LOCALAPPDATA\Dcrd\wbc.conf
PS> cp $env:GOPATH\src\github.com\wbcoin\dcrwallet\sample-dcrwallet.conf $env:LOCALAPPDATA\Dcrwallet\dcrwallet.conf
PS> $editor $env:LOCALAPPDATA\Dcrd\wbc.conf
PS> $editor $env:LOCALAPPDATA\Dcrwallet\dcrwallet.conf
```
...
```bash
$ cp $GOPATH/src/github.com/wbcoin/wbc/sample-wbc.conf ~/.wbc/wbc.conf
$ cp $GOPATH/src/github.com/wbcoin/dcrwallet/sample-dcrwallet.conf ~/.dcrwallet/dcrwallet.conf
$ $EDITOR ~/.wbc/wbc.conf
...
The [integrated github issue tracker](https://github.com/wbcoin/dcrwallet/issues)
