...
type TicketPurchaser struct {
	...
	wbcChainSvr      *dcrrpcclient.Client
...
func (t *TicketPurchaser) Purchase(height int64) (*PurchaseStats, error) {
	...
	if refreshStakeInfo {
		...
		for i := 1; i <= stakeInfoReqTries; i++ {
			curStakeInfo, err = t.wallet.StakeInfo(t.wbcChainSvr)
			if err != nil {
	...
	sDiffEsts, err := t.wbcChainSvr.EstimateStakeDiff(nil)
	if err == nil {
	...
	oneBlock := uint32(1)
	info, err := t.wbcChainSvr.TicketFeeInfo(&oneBlock, &zeroUint32)
	if err != nil {
		return ps, err
	}
