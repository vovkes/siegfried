...
func (t *TicketPurchaser) ownTicketsInMempool() (int, error) {
	...
	if t.votingAddress != nil {
		tiHashes, err := t.wbcChainSvr.GetRawMempool(dcrjson.GRMTickets)
		if err != nil {
			return 0, err
		}
		...
		for i := range tiHashes {
			raw, err := t.wbcChainSvr.GetRawTransactionVerbose(tiHashes[i])
			if err != nil {
				return 0, err
			}
...
func (t *TicketPurchaser) allTicketsInMempool() (int, error) {
	tfi, err := t.wbcChainSvr.TicketFeeInfo(&zeroUint32, &zeroUint32)
	if err != nil {
		return 0, err
	}
