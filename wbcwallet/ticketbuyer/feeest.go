...
func (t *TicketPurchaser) findClosestFeeWindows(difficulty dcrutil.Amount,
	...
	info, err := t.wbcChainSvr.TicketFeeInfo(&zeroUint32, &wtcUint32)
	if err != nil {
		return 0.0, err
	}
	...
	for i := range info.FeeInfoWindows {
		...
		blH, err := t.wbcChainSvr.GetBlockHash(startHeight)
		if err != nil {
			return 0, err
		}
		blkHeader, err := t.wbcChainSvr.GetBlockHeader(blH)
		if err != nil {
			return 0, err
		}
...
func (t *TicketPurchaser) findTicketFeeBlocks(useMedian bool) (dcrutil.Amount, error) {
	...
	info, err := t.wbcChainSvr.TicketFeeInfo(&btaUint32, nil)
	if err != nil {
		return 0.0, err
	}
