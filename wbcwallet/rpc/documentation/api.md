...
the management of the wallet and its connection to the WBC network.  It has
...
dynamically start the wbc RPC client.  This RPC client is used for wallet
syncing and publishing transactions to the WBC network.
...
- `string network_address`: The host/IP and optional port of the RPC server to
  ...
  default port is chosen corresponding to the default wbc RPC port of the
  active WBC network.
...
- `string public_key`: The public key encoded as a string in the WBC encoding
  format.
...
the WBC network.  If the transaction spends any of the wallet's unspent
...
notifications passed from the JSON RPC of wbc. Right now, wallet block processing
