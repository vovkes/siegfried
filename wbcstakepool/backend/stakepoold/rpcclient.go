...
func connectNodeRPC(ctx *appContext, cfg *config) (*rpcclient.Client, semver, error) {
	...
	wbcCert, err := ioutil.ReadFile(cfg.DcrdCert)
	if err != nil {
		log.Errorf("Failed to read wbc cert file at %s: %s\n",
			cfg.DcrdCert, err.Error())
		return nil, nodeVer, err
	}

	log.Debugf("Attempting to connect to wbc RPC %s as user %s "+
	...
	connCfgDaemon := &rpcclient.ConnConfig{
		...
		Certificates: wbcCert,
	}

	ntfnHandlers := getNodeNtfnHandlers(ctx, connCfgDaemon)
	wbcClient, err := rpcclient.New(connCfgDaemon, ntfnHandlers)
	if err != nil {
		log.Errorf("Failed to start wbc RPC client: %s\n", err.Error())
		return nil, nodeVer, err
	}

	// Ensure the RPC server has a compatible API version.
	ver, err := wbcClient.Version()
	if err != nil {
		log.Error("Unable to get RPC version: ", err)
		return nil, nodeVer, fmt.Errorf("Unable to get node RPC version")
	}

	wbcVer := ver["wbcjsonrpcapi"]
	nodeVer = semver{wbcVer.Major, wbcVer.Minor, wbcVer.Patch}
