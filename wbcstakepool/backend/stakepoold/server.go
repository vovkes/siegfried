...
func runMain() error {
	...
	if err != nil || nodeConn == nil {
		log.Infof("Connection to wbc failed: %v", err)
		return err
	}
	...
	if err != nil {
		log.Errorf("Unable to get current network from wbc: %v", err)
		return err
	}
	log.Infof("Connected to wbc (JSON-RPC API v%s) on %v",
	...
	for {
		...
		if err != nil {
			log.Errorf("unable to get bestblock from wbc: %v", err)
			return err
		}
	...
	log.Info("subscribed to notifications from wbc")
