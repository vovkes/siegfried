...
type config struct {
	...
	DcrdHost         string  `long:"wbchost" description:"Hostname/IP for wbc server"`
	DcrdUser         string  `long:"wbcuser" description:"Username for wbc server"`
	DcrdPassword     string  `long:"wbcpassword" description:"Password for wbc server"`
	DcrdCert         string  `long:"wbccert" description:"Certificate path for wbc server"`
...
func loadConfig() (*config, []string, error) {
	...
	if len(cfg.DcrdHost) == 0 {
		str := "%s: wbchost is not set in config"
		err := fmt.Errorf(str, funcName)
		fmt.Fprintln(os.Stderr, err)
		return nil, nil, err
	}
	...
	if len(cfg.DcrdCert) == 0 {
		str := "%s: wbccert is not set in config"
		err := fmt.Errorf(str, funcName)
		fmt.Fprintln(os.Stderr, err)
		return nil, nil, err
	}
	...
	if len(cfg.DcrdUser) == 0 {
		str := "%s: wbcuser is not set in config"
		err := fmt.Errorf(str, funcName)
		fmt.Fprintln(os.Stderr, err)
		return nil, nil, err
	}
	...
	if len(cfg.DcrdPassword) == 0 {
		str := "%s: wbcpassword is not set in config"
		err := fmt.Errorf(str, funcName)
		fmt.Fprintln(os.Stderr, err)
		return nil, nil, err
	}
	...
	if !fileExists(cfg.DcrdCert) {
		...
		if !fileExists(path) {
			str := "%s: wbccert " + cfg.DcrdCert + " and " +
				path + " don't exist"
			err := fmt.Errorf(str, funcName)
			fmt.Fprintln(os.Stderr, err)
			return nil, nil, err
